...
//      pinIconUrl: Optional.of('https://webpop.github.io/jquery.pin/images/pin.png'),
  ...
  static final _mockGeoObjectsDetails = <GeoObjectDetails>[
    GeoObjectDetails(
      ...
      latitude: _geoObjectLocations[0].latitude,
      longitude: _geoObjectLocations[0].longitude,
      imageUrl: Optional.of('images/mock_images/geo_0.jpg'),
    ...
    GeoObjectDetails(
      ...
      latitude: _geoObjectLocations[1].latitude,
      longitude: _geoObjectLocations[1].longitude,
      imageUrl: Optional.of('images/mock_images/geo_1.jpg'),
    ...
    GeoObjectDetails(
      ...
      latitude: _geoObjectLocations[2].latitude,
      longitude: _geoObjectLocations[2].longitude,
      imageUrl: Optional.of('images/mock_images/geo_2.jpg'),
    ...
    GeoObjectDetails(
      ...
      latitude: _geoObjectLocations[3].latitude,
      longitude: _geoObjectLocations[3].longitude,
      imageUrl: Optional.of('images/mock_images/geo_3.jpg'),
    ...
    GeoObjectDetails(
      ...
      latitude: _geoObjectLocations[4].latitude,
      longitude: _geoObjectLocations[4].longitude,
      imageUrl: Optional.of('images/mock_images/geo_4.jpg'),
    ...
    GeoObjectDetails(
      ...
      latitude: _geoObjectLocations[5].latitude,
      longitude: _geoObjectLocations[5].longitude,
      imageUrl: Optional.of('images/mock_images/geo_5.jpg'),
    ...
    GeoObjectDetails(
      ...
      latitude: _geoObjectLocations[6].latitude,
      longitude: _geoObjectLocations[6].longitude,
      imageUrl: Optional.of('images/mock_images/geo_6.jpg'),
    ...
    GeoObjectDetails(
      ...
      latitude: _geoObjectLocations[7].latitude,
      longitude: _geoObjectLocations[7].longitude,
      imageUrl: Optional.of('images/mock_images/geo_7.jpg'),
    ...
    switch (_currentDay) {
      ...
      case '1':
        return _mockGeoObjects
            .where((it) => ['0', '1', '2', '3', '4', '5', '6', '7'].contains(it.ref))
            .toList();
      // Today
      case '2':
        return _mockGeoObjects.where((it) => ['0', '1', '2', '3'].contains(it.ref)).toList();

      // Tomorrow
      case '3':
        return _mockGeoObjects.where((it) => ['4', '5', '6', '7'].contains(it.ref)).toList();
  ...
  Future<List<SearchSuggestion>> searchGeoObjects(String key) async {
    ...
    if (key.isEmpty) {
      ...
      switch (_currentDay) {
        case '1':
          details =
              _mockGeoObjectsDetails.where((it) => ['0', '1', '2', '3'].contains(it.ref)).toList();
          break;

        case '2':
          details =
              _mockGeoObjectsDetails.where((it) => ['4', '5', '6', '7'].contains(it.ref)).toList();
          break;
  ...
  Future<void> _boundByPoints(Point srcPoint, Point destPoint) async {
    ...
    await _yandexMapController.setBounds(
      southWestPoint: Point(
        latitude: math.min(srcPoint.latitude, destPoint.latitude) - boundaryBorder,
        longitude: math.min(srcPoint.longitude, destPoint.longitude) - boundaryBorder,
      ),
      northEastPoint: Point(
        latitude: math.max(srcPoint.latitude, destPoint.latitude) + boundaryBorder,
        longitude: math.max(srcPoint.longitude, destPoint.longitude) + boundaryBorder,
      ),
    );
  }

  @override
  Future<RouteInfo> buildMasstransitRoute(Point srcPoint, Point destPoint) async {
  ...
  Future<Map<RouteType, String>> estimateRoutes(
    ...
    return {
      RouteType.masstransit:
          await _yandexMapController.estimateMasstransitRoute(src: srcPoint, dest: destPoint),
      RouteType.pedestrian:
          await _yandexMapController.estimatePedestrianRoute(src: srcPoint, dest: destPoint),
      RouteType.bicycle:
          await _yandexMapController.estimateBicycleRoute(src: srcPoint, dest: destPoint),
      RouteType.driving:
          await _yandexMapController.estimateDrivingRoute(src: srcPoint, dest: destPoint),
    };
  }
  ...
  Future<void> _moveBoundaries(Iterable<GeoObject> geoObjects, bool animation) async {
    animation = false;
...
//        final response = await http.get(geoObject.pinIconUrl.value);
        final ByteData bytes = await rootBundle.load(geoObject.pinIconUrl.value);
        final Uint8List image = bytes.buffer.asUint8List();
        await _yandexMapController.addPlacemark(
          Placemark(
            point: point,
            onTap: onTap,
            rawImageData: image, //response.bodyBytes,
            opacity: 1.0,
          ),
        );
      } else {
